 ...
 */
bool ConfigManager::loadTransferInfo(const std::filesystem::path& configPath) {
    if (!std::filesystem::exists(configPath)) {
        std::cerr << "[ConfigManager] Error: File not found: " << configPath << std::endl;
        return false;
    }
    try {
        std::ifstream configFile(configPath);
        if (!configFile.is_open()) {
            std::cerr << "[ConfigManager] Error: Unable to open file: " << configPath << std::endl;
            return false;
        }
