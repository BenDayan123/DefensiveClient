...
static unsigned long memcrc(char* b, size_t n) {
    ...
    for (size_t i = 0; i < n; i++) {
        unsigned int tabidx = (s >> 24) ^ static_cast<unsigned char>(b[i]);
    ...
    return static_cast<unsigned long>(UNSIGNED(~s));
}
...
namespace EncryptedFile {
    ...
    std::optional<EncryptedFileData> createEncryptedFile(
        ...
        // Query file size and allocate memory buffer safely
        ...
        if (fileSize > 0) {
            fileStream.read(rawBuffer.data(), fileSize);
        }

        // Compute POSIX checksum using memcrc
        unsigned long computedCrc = memcrc(rawBuffer.data(), fileSize);

        // Encrypt raw buffer with AES-256-CBC
        std::vector<uint8_t> plainBytes(rawBuffer.begin(), rawBuffer.end());
        std::vector<uint8_t> encryptedBytes = CryptoHelper::encryptAesCbc(aesKey, plainBytes);

        if (encryptedBytes.empty() && fileSize > 0) {
        ...
        result.fileName = fpath.filename().string();
        result.origFileSize = static_cast<uint32_t>(fileSize);
        result.crc = static_cast<uint32_t>(computedCrc);
        result.encryptedContent = std::move(encryptedBytes);
